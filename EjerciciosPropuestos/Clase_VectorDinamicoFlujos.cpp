...
private:
    ...
    void copiarDesde(const VectorDinamico& vector);
    void libera();
};
...
void VectorDinamico::copiarDesde(const VectorDinamico& vector){
    size=vector.size;
    if(size>0){
        ptr = new int[ size ]; // crea espacio para el array de elementos
        for ( int i = 0; i < size; ++i )
...
istream& operator>>(istream &flujo, VectorDinamico &vector){
...
void VectorDinamico::Imprime_reves()
{
...
int main()
{
   ...
   VectorDinamico vector;
    ...
    cout << "Introduce enteros " << endl;
    // while (cin >> vector)  Implementacion de '>>' WTF?!
    {
        cout << "Introduce enteros " << endl;
        // vector.setElement(x);
    }

    vector.Imprime_reves();
}
